...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (animationSettings.animator == null)
    ...
    void Update()
    {
        ...
        animationSettings.animator.SetBool("Grounded", isGroundedNow);
    ...
    {
        if (animationSettings.animator == null) return;

        // Set movement animation
        bool isMoving = Mathf.Abs(movementState.horizontal) > 0.1f;
        animationSettings.animator.SetBool(animationSettings.movingAnimParam, isMoving && !attackState.isAttacking);

        // Idle is handled by transitions in the Animator
        animationSettings.animator.SetBool(animationSettings.idleAnimParam, !isMoving && !attackState.isAttacking);
    ...
    private IEnumerator PerformAttack()
    {
        ...
        visualState.isInvuln = true; // Temporarily set invulnerability during attack
        ...
        if (animationSettings.animator != null)
        {
            animationSettings.animator.SetTrigger(animationSettings.attackAnimParam);
        }
        ...
        visualState.isInvuln = false; // Reset invulnerability state
    }
    ...
    IEnumerator InvulnerabilityCoroutine()
    {
        ...
        if (characterSprite != null)
        {
            ...
            {
                characterSprite.color = Color.black; // Flash color
                yield return new WaitForSeconds(settings.invulnerabilityTime / numberOfFlashes / 2);
                characterSprite.color = originalColor; // Reset color
                yield return new WaitForSeconds(settings.invulnerabilityTime / numberOfFlashes / 2);
            }
        }
